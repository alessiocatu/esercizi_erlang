...
start(M, N, Msg) ->
  register(fst, spawn(?MODULE, create, [N, 1, self()])),
  io:format("*** [fst] is at ~p~n", [whereis(fst)]),
  receive
    ready -> ok
    after 50000 -> exit(timeout)
  end,
  msg_dispacher(M, 1, Msg),
  fst!stop,
  ok.

create(1, Who, Starter) ->
  Starter ! ready,
  io:format("*** created [~p] as ~p connected to ~p~n", [self(), Who, fst]),
  loop_last(fst, Who);
...
loop_last(Next, Who) ->
  receive
    {Msg, N, Pass} ->
      ...
      io:format("*** [~p] is ~p alive ~p [loop_last]~n", [Who, Next, erlang:is_process_alive(Next)]),
      Next ! {Msg, N, Pass+1},
    ...
    stop ->
      ...
      exit(normal),
      unregister(fst),
      io:format("*** [~p] unregistered [~p]~n", [Who, fst]),
      io:format("# Actor ~p stops. [loop_last]~n", [Who]);
    Other -> io:format("other last ~p~n", [Other])
  end.

msg_dispacher(M, M, Msg) -> fst!{Msg, M, 1};
msg_dispacher(M, N, Msg) -> fst!{Msg, N, 1}, msg_dispacher(M, N+1, Msg).
